...
\noindent
%%% TO BE WRITTEN LAST.
%%% Structure: degeneracy framing (1 sentence) | system (1 sentence) |
%%% Level 1-2 result (1 sentence) | Level 3-4 result (1 sentence) |
...
functional sensitivity commitment: specialists develop
$\statDegAsixVarRatio\times$ higher sensitivity variance than generalists
trained on all mice simultaneously, despite exploring statistically
indistinguishable topological diversity. Critically, this functional commitment is itself degenerate:
replicates of the same mouse achieve equivalent specialization through different
sensitivity profiles, confirming that degeneracy operates not just at the
structural level but at the level of the computational strategies that implement
individual behavior.
\end{abstract}
...
the same function \citep{edelman2001degeneracy, marder2006variability}. Here we
use the term in a broadened sense that also encompasses its complement---%
structurally indistinguishable circuits supporting distinct individual
outputs---a regime adjacent to what \citet{albantakis2024degeneracy} term
plurifunctionality. It is a fundamental organizational principle of biological
nervous systems. In naturally
...
individual-target selection shapes instead is committed functional sensitivity:
specialists develop $\statDegAsixVarRatio\times$ higher sensitivity variance
than generalists trained on all mice simultaneously, despite exploring
statistically indistinguishable topological diversity. Critically, this functional commitment is
itself degenerate: replicates of the same mouse achieve equivalent behavioral
specialization through distinct sensitivity profiles. Selection navigates a
degenerate space to produce individual-specific function without leaving a
legible structural trace.
...
regime (E/(E+I) $\approx \statEiBalanceSpeedMean$) uniformly across all 9 mice, consistent with an
open-loop tonic drive. The turn motor neuron converged to a more balanced ratio
($\approx \statEiBalanceTurnMedian$) with greater variability between mice, consistent with
closed-loop control in which inhibitory feedback modulates turning direction,
an organisation analogous to push-pull motor circuits in biological locomotion
\citep{kiehn2006locomotor} (Figure~\ref{fig:system}H;
...
\begin{figure}[H]
  ...
  converges uniformly to $\approx\statEiBalanceSpeedMean$ excitatory across all mice;
  turn motor (Trn) converges to $\approx\statEiBalanceTurnMedian$ with greater
...
$\statCrossMouseSpecIndex$ at generation~150 (Figure~\ref{fig:paradox}E), while
...
same mouse: within-mouse sensitivity profiles are significantly \emph{less}
similar to each other than to profiles from a different mouse (pairwise cosine
similarity within-mouse $\statClaimTwentyEightWithinSim$ versus between-mouse
$\statClaimTwentyEightBetweenSim$; Mann-Whitney $p = \statClaimTwentyEightMwP$),
so replicates of the same mouse commit to distinct pathways rather than
converging on a shared subset. Degeneracy thus operates even within a single
behavioral target: replicates of the same mouse are no more aligned in
sensitivity space than agents trained on different mice---indeed marginally
\emph{more} idiosyncratic---ruling out a shared, mouse-specific pathway
signature. Selection builds the strength of functional commitment, not convergence to
specific pathways. Generalist agents show substantially lower sensitivity variance
...
The fitness cost confirms that generalists do not simply fail to converge: a mean
cost of $\statDegAsixMeanCostPct$\% implies generalists achieve ${\sim}\statDegAsixGenPerfPct$\% of
specialist performance on average, confirming clear behavioral learning. The low
...
a committed functional sensitivity structure: high sensitivity variance across replicates,
with the specific committed pathways varying freely. Behavioral individuation in this
constrained system is thus encoded in functional sensitivity, not in circuit structure,
and this functional encoding is itself degenerate: the same behavioral specialization
can be achieved through distinct sensitivity commitments.
...
\begin{figure}[H]
  ...
  ratio $= \statDegAsixVarRatio\times$; Mann-Whitney $p = \statDegAsixVarMwP$, one-sided).
...
Degeneracy is typically discussed as a property that must be inferred from
behavioral equivalence under structural variation. Our constrained system makes it
an architectural fact: the constraint geometry (Dale's Law, degree limits, quantized
weights) compresses all solutions into the same structural region regardless of the
behavioral target, so degeneracy is present by construction at the aggregate level.
What our analyses reveal is how far this degeneracy extends: it spans every level of
circuit description examined, from aggregate statistics and connection topology through
causal sensitivity, representational geometry, and dynamical geometry.

It extends further than the aggregate floor. Connection topology varies freely within
and across mice, yet that variation carries no information about behavioral identity
at any scale: within mice, between mice, or across the full agent population. The
...
within-mouse sensitivity profiles are significantly \emph{less} similar than between-mouse profiles
($\statClaimTwentyEightWithinSim$ versus $\statClaimTwentyEightBetweenSim$;
Mann-Whitney $p = \statClaimTwentyEightMwP$), so even replicates of a single mouse
...
$\statDegAsixVarRatio\times$ difference in sensitivity variance between specialists
and generalists is the signature of what individual-target selection achieves in a
...
may be many-to-many rather than one-to-one (here demonstrated directly in the
many-structures-to-one-function, i.e.\ degenerate, direction; the converse
plurifunctional direction follows by extension), particularly in systems where
architectural constraints compress the structural space \citep{bargmann2013connectome,
jonas2017microprocessor}.
...
\paragraph{Connection-level analysis.}
Per-connection ANOVA tested only connections that were non-zero in at least one agent, to
avoid inflating the multiple-comparison correction with structurally impossible entries
(the $14 \times 14$ weight matrix contains many entries that are zero in all agents due to
sparsity constraints and the prohibition on incoming connections to sensory neurons).
FDR correction was applied across all tested connections.
...
for 400 steps; $\lambda_1$ was estimated as the linear slope over the first 200 steps
(tanh saturation in this small network ensures the linear regime is reliably reached within
this window). One-way ANOVA and Kruskal-Wallis tests compared $\lambda_1$
across mice.
...
mice; for generalists, across the 6 generalist replicates. The
$\statDegAsixVarRatio\times$ ratio between specialist and generalist mean sensitivity
variance reflects this normalised comparison; its significance was assessed by a
one-sided Mann-Whitney $U$ test on the two sets of per-neuron variances
($n = 14$ neurons per group, specialist $>$ generalist; $p = \statDegAsixVarMwP$).
...
The Lyapunov exponent was obtained as the slope of the mean log-norm growth across
all 8 directions and the 400 steps:
\begin{equation}
  \lambda_1 = \frac{1}{400}\,\frac{1}{8}
  \sum_{k=1}^{8} \log \frac{\|\delta \mathbf{h}_t^{(k)}\|}{\|\delta \mathbf{h}_0^{(k)}\|}.
\end{equation}
...
uniformly to a strongly excitatory-dominated regime ($\approx\statEiBalanceSpeedMean$)
across all 9 mice (panel B). The turn motor neuron converges to a more balanced
regime ($\approx\statEiBalanceTurnMedian$) with greater inter-mouse variability
...
\begin{figure}[H]
  ...
  $\approx\statEiBalanceSpeedMean$ excitatory across all 9 mice.
  (C)~E/I balance for the turn motor neuron, converging to
  $\approx\statEiBalanceTurnMedian$ with greater variability, consistent with
...
$\statHoldoutTrainingIndex$ on the training set. The attenuation likely reflects
reduced statistical precision of the held-out behavioral baseline: each mouse's
held-out target is estimated from ${\approx}\statHoldoutMeanBoutsHoldout$
trajectory bouts rather than ${\approx}\statHoldoutMeanBoutsTotal$, reducing the
resolution of the behavioral fingerprint. No agent lost specialisation entirely;
the unanimous persistence of positive indices rules out genuine overfitting.
