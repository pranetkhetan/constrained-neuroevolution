...
to carry no shared mouse-specific signature, so behavioral individuation is expressed as a
magnitude of functional commitment rather than a structural or
pathway fingerprint---degeneracy that operates not just at the structural level but
...
a circuit commits to carry no shared mouse-specific signature, so individuation
...
\begin{figure}[H]
  ...
  points overlaid). Generalists carry no own-mouse peak: their mean $\Delta$fit
  ($\statGeneralistGrandMean$, 15 replicates) sits at the specialist
  \emph{other}-mouse level ($\statPermutationOptionaSpecOtherMean$), well below the
  specialist \emph{own}-mouse level ($\statPermutationOptionaSpecOwnMean$), whereas
  specialists show unanimous own $>$ other (Wilcoxon $p = \statPhaseThreebWilcoxonP$).
  Generalist $\Delta$fit does vary across the nine test mice
  (Kruskal-Wallis $p = \statGeneralistKruskalP$), but this reflects intrinsic
  between-mouse disruptability shared identically across replicates (Friedman
  $p = \statGeneralistFriedmanP$ for a consistent per-mouse rank order), not a
  per-individual calibration signature.
...
what individual-target selection actually achieves. We trained fifteen generalist agents
(independent replicates) simultaneously on all 9 mice, using fitness equal to the mean
across all 9 behavioral targets, with the same architecture, constraints, and evolutionary
parameters as the per-mouse runs.
...
$= \statDegAsixVarRatioWithin$--$\statDegAsixVarRatio\times$, within-mouse-pooled to
across-all-54 definitions; per-neuron Mann-Whitney $p = \statDegAsixVarMwP$,
...
Because the ratio's magnitude depends on the generalist replicate sample, we estimated
it over fifteen independent generalist replicates and quantified its uncertainty with a
hierarchical bootstrap (95\% CI $[\statDegAsixVarBootCiLo,\,\statDegAsixVarBootCiHi]$);
treating the mouse as the independent unit, all $\statDegAsixMouseNPos$ of 9 mice exceed
the generalist (one-sample Wilcoxon $p = \statDegAsixMouseWilcoxP$), so the direction is
unambiguous even though the point magnitude carries wide intervals.
...
permutation profile confirms the mechanism: its mean $\Delta$fit
($\statGeneralistGrandMean$, 15 replicates) sits at the specialist \emph{other}-mouse
level ($\statPermutationOptionaSpecOtherMean$)---not the specialist \emph{own}-mouse
level ($\statPermutationOptionaSpecOwnMean$)---so the generalist carries no own-mouse
peak, in contrast to specialists whose own-mouse degradation is unanimous
(own $>$ other in 8/9 mice, Wilcoxon $p = \statPhaseThreebWilcoxonP$;
Figure~\ref{fig:causal}D). Generalist $\Delta$fit does differ across the nine test
mice (Kruskal-Wallis $p = \statGeneralistKruskalP$), but this is an intrinsic
between-mouse disruptability effect---the same mice are hardest to perturb in every
replicate (Friedman $p = \statGeneralistFriedmanP$)---rather than a per-individual
calibration signature. A generalist circuit is thus disrupted like a specialist
evaluated on its wrong mouse: partially calibrated to everyone, fully calibrated to
none.
...
identity (A4: which neurons an agent is sensitive to carries no consistent mouse-specific signature).
...
pathway perturbations, with no shared mouse-specific pattern in which pathways are committed to.
...
onward (Supplementary Figure~\ref{fig:supp_sens_commitment_evo}); the small within-
versus between-mouse gap in sensitivity-profile similarity does not survive a
pseudoreplication-aware test (§\ref{sec:sensitivity}), so it is not evidence that
replicates commit to distinct pathways.
...
pathways carry no consistent mouse-specific signature. Behavioral individuation in this constrained
...
per mouse) are planned. More generally, with six replicates per mouse a non-significant
between-mouse contrast could in principle reflect limited statistical power rather than
genuine equivalence. The structural, representational, and dynamical degeneracy nulls are
guarded against this because each is paired with a matched random-agent architectural
control that reproduces the same null---locating the uniformity in the constrained
architecture rather than in insufficient sampling---while the aggregate-feature (0/18)
null is additionally backed by the large-effect power analysis
(Supplementary Figure~\ref{fig:supp_power}).
...
mice; for generalists, across the 15 generalist replicates. The within-mouse-pooled
...
archive and intermediate analysis outputs---including the aggregate individuation result
(\texttt{analysis/individuation\_results.pkl}) that underlies the classifier accuracy
reported in the Discussion and Supplementary Analysis~\ref{sec:supp_holdout}---are deposited
on Zenodo (DOI: \texttt{[ZENODO-DOI]}) and travel with the repository.
Raw mouse behavioral data are the tracked trajectory (\texttt{-tf}) files from
\citet{rosenberg2021mice} (eLife 2021; \texttt{doi:10.7554/eLife.66175}), which we do not
redistribute. To regenerate the individuation result from source, obtain those files, place
them as \texttt{data/raw/\{mouse\}-tf} for the nine mice, and run
\texttt{python scripts/analyze\_individuation.py -{}-tf\_dir data/raw -{}-n\_folds 6
-{}-fold\_mode stride} (deterministic, seed~$=1$); all other results regenerate from the
shipped intermediates via \texttt{python scripts/build\_paper\_stats.py}.
...
the same mouse (solid) vs between mice (dashed). Within-mouse cosine similarity is
marginally below the between-mouse baseline over training, but this small gap is not
significant under a pseudoreplication-aware test (label-permutation
$p = \statClaimTwentyEightPermP$; §\ref{sec:sensitivity}); we therefore read it only as
the absence of a shared mouse-specific pathway signature, not as evidence that
replicates commit to distinct pathways.
...
\begin{figure}[H]
  ...
  $\pm$ SD shading) vs between-mouse baseline (dashed). Within-mouse similarity is
  marginally below the between-mouse baseline; this small gap is not significant under a
  pseudoreplication-aware test (label-permutation $p = \statClaimTwentyEightPermP$;
  §\ref{sec:sensitivity}), indicating no shared mouse-specific pathway signature
  rather than convergence to distinct per-replicate solutions.}
